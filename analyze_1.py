...
# Dictionary mapping vulnerability names to their severity levels
# Source(Slither): https://github.com/crytic/slither/wiki/Detector-Documentation#
gravity = {
...
def revise():
    ...
    for m in data:
        ...
        for p in data[m]:
            ...
            for it in data[m][p]:
                if (data[m][p][it]["response"] != "error"
                        and data[m][p][it]["compilation"]['returnCode'] == 0):
                    compiledTotal += 1
                    compiled += 1
        ...
        if (compiledTotal + notCompiledTotal) == 0:
            stats[m]['compilation']['ratio'] = 0
        else:
            stats[m]['compilation']['ratio'] = (
                stats[m]['compilation']['ok'] / (compiledTotal + notCompiledTotal) * 100
            )
